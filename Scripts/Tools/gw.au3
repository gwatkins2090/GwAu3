...
Local $mTraderCostID, $mTraderCostValue
...
Func Initialize($aGW, $bChangeTitle = True, $aUseStringLog = False)
	Local $lWinList
	...
	$mDisplayLanguage = $mBasePointer - 220
	SetValue('DisplayLanguage', '0x' & Hex($mDisplayLanguage, 8))
...
Func AddPattern($aPattern)
	Local $lSize = 0.5 * StringLen($aPattern)
...
Func IdentifyItem($aItem)
	...
	Return SendPacket(0xC, 0x65, $lIDKit, $lItemID)
EndFunc   ;==>IdentifyItem

;~ Description: Identifies all items in a bag.
Func IdentifyBag($aBag, $aWhites = False)
	Local $lItem
...
;~ Description: Request a quote to buy an item from a trader.
Func TraderRequest($aItem)
	Local $lMerchantItemsBase = GetMerchantItemsBase()
	Local $lMerchantItemsSize = GetMerchantItemsSize()

	If Not $lMerchantItemsBase Or Not $lMerchantItemsSize Then Return
	If $aItem < 1 Or $aItem > $lMerchantItemsSize Then Return

	DllStructSetData($mRequestQuote, 2, MemoryRead($lMerchantItemsBase + 4 * ($aItem - 1)))
...
;~ Description: Enable or disable a skill on a hero's skill bar.
...
Func MoveTo($aX, $aY, $aRandom = 50)
	...
	$lMe = GetAgentByID(-2)
	Do
		$lOldX = DllStructGetData($lMe, 'X')
		$lOldY = DllStructGetData($lMe, 'Y')
		...
		$lMe = GetAgentByID(-2)
		If $lOldX == DllStructGetData($lMe, 'X') And $lOldY == DllStructGetData($lMe, 'Y') Then
			$lBlocked += 1
			Move($aX, $aY, $aRandom)
		EndIf
	Until ComputeDistance(DllStructGetData($lMe, 'X'), DllStructGetData($lMe, 'Y'), $aX, $aY) < 250 Or $lBlocked > 14
EndFunc   ;==>MoveTo
...
Func GetImperialFaction()
	Local $lOffset[4] = [0, 0x18, 0x2C, 0x70C]
	Local $lReturn = MemoryReadPtr($mBasePointer, $lOffset)
	Return $lReturn[1]
...
Func GetItemBySlot($aBag, $aSlot)
	...
	Local $lItemStruct = DllStructCreate('long id;long agentId;byte unknown1[4];ptr bag;ptr modstruct;long modstructsize;ptr customized;byte unknown2[4];byte type;byte unknown3;short extraId;short value;byte unknown4[2];short interaction;long modelId;ptr modString;byte unknown5[4];ptr NameString;byte unknown6[15];byte quantity;byte unknown7[2];byte slot')
...
Func FindSalvageKit()
	For $i = 1 To 16
		For $j = 0 To DllStructGetData(GetBag($i), 'Slots') - 1
			...
			If DllStructGetData($lItemInfo, 'ModelID') = 2991 Or DllStructGetData($lItemInfo, 'ModelID') = 2992 Then Return DllStructGetData($lItemInfo, 'ID')
		Next
	Next
...
Func FindIDKit()
	For $i = 1 To 16
		For $j = 0 To DllStructGetData(GetBag($i), 'Slots') - 1
			$lItemInfo = GetItemBySlot($i, $j)
			If DllStructGetData($lItemInfo, 'ModelID') = 2989 Or DllStructGetData($lItemInfo, 'ModelID') = 5899 Then Return DllStructGetData($lItemInfo, 'ID')
		Next
	Next
EndFunc   ;==>FindIDKit

;~ Description: Returns the quote ID.
...
;~ Description: Test if a quote has been requested.
Func TraderCheck($aID, $aValue)
	If GetTraderCostID() = $aID And GetTraderCostValue() = $aValue Then
		Return True
	Else
		Return False
	EndIf
...
Func GetIsHeroSkillSlotDisabled($aHeroNumber, $aSkillSlot)
	...
	$lOffset[2] = 0x2C
	$lOffset[3] = 0x68C
	$lOffset[4] = 0x160 + 0xBC * ($aHeroNumber - 1)
	Local $lFlag = MemoryReadPtr($mBasePointer, $lOffset)
	If BitAND(2 ^ ($aSkillSlot - 1), $lFlag[1]) Then Return 1
	Return 0
...
;~ Description: Returns the recharge timestamp of an equipped skill.
...
Func GetAttributeByID($aAttributeID, $aWithRunes = False)
	Local $lOffset[5] = [0, 0x18, 0x2C, 0xAC, 0x8 + 20 * $aAttributeID]
	Local $lAttributeAddress = MemoryReadPtr($mBasePointer, $lOffset, 'wchar[30]')

	If $aWithRunes Then
		Return MemoryRead($lAttributeAddress[0])
	Else
		Return MemoryRead($lAttributeAddress[0] + 4)
	EndIf
...
Func WaitMapLoading($aMapID = 0, $aDeadlock = 15000)
	...
	Do
		Sleep(200)
		$lMapLoading = GetMapLoading()
		If TimerDiff($lDeadlock) > $aDeadlock And $lMapLoading < 2 Then Return False
...
Func GetQuestByID($aQuestID = 0)
	Local $lQuestStruct = DllStructCreate('long id;long LogState;byte unknown1;byte unknown2;byte unknown3;long MapFrom;float X;float Y;byte unknown4[8];long MapTo')
...
Func GetDisplayLanguage()
	...
	Local $lResult = MemoryReadPtr($mDisplayLanguage, $lOffset)
	Return $lResult[1]
...
Func CreateMain()
	...
	_('mov ecx,dword[DisplayLanguage]')
	...
	For $i = 1 To StringLen($lTempASM)
		...
		Switch $lToken
			Case '(', '[', '{'
				$lInExpression = True
			Case ')'
				$mASMString &= Hex(GetLabelInfo($lExpression) - $lCurrentOffset - 1, 2)
				$lCurrentOffset += 1
				$lInExpression = False
				$lExpression = ''
			Case ']'
				...
				$lExpression = ''
			Case '}'
				$mASMString &= SwapEndian(Hex(GetLabelInfo($lExpression) - $lCurrentOffset - 4, 8))
				$lCurrentOffset += 4
				$lInExpression = False
				$lExpression = ''
			Case Else
